...
export function TeamPage() {
  ...
  useEffect(() => { getTeam().then(setMembers).catch(() => setMembers([])); }, []);
...
export function TeamMemberPage({ id }: { id: string }) {
  ...
  useEffect(() => { getTeamMember(id).then(setMember).catch((reason) => setError(reason instanceof Error ? reason.message : "Team member not found.")); }, [id]);
