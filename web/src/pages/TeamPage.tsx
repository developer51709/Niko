...
export function TeamPage() {
  ...
  return <><style>{teamStyles}</style><PublicHeader page="team" /><main className="shell page-main">
    ...
    <div className="team-grid">{members.map((member) => <button className="team-card" key={member.id} onClick={() => navigate(`/team/${member.id}`)}>
      ...
      <div className="team-card-body">{avatar(member) ? <img className="team-avatar" src={avatar(member)!} alt="" /> : <span className="team-avatar team-avatar-fallback">{member.name.slice(0, 1)}</span>}<div><span className="team-role">{member.role_label}</span><h2>{member.name}</h2><p>{member.bio || "Part of the Niko team."}</p></div></div>
...
export function TeamMemberPage({ id }: { id: string }) {
  ...
  return <><style>{teamStyles}</style><PublicHeader page="team" /><main className="shell page-main team-profile-page">{error ? <div className="empty-state">{error}</div> : !member ? <div className="section-loading">Loading profile…</div> : <>
    ...
    <section className="team-profile-card">{avatar(member) ? <img className="team-profile-avatar" src={avatar(member)!} alt="" /> : <span className="team-profile-avatar team-avatar-fallback">{member.name.slice(0, 1)}</span>}<div className="team-role">{member.role_label}</div><h1>{member.name}</h1><p className="team-profile-handle">{member.username ? `@${member.username}` : "Niko staff"} · <span className={`status-${member.status || "offline"} ${member.status === "online" ? "is-online" : ""}`}>{statusLabel(member)}</span></p><p className="team-profile-bio">{member.bio || "This team member has not added an extended introduction yet."}</p>{(member.public_links || []).length > 0 && <nav className="team-profile-links" aria-label={`${member.name}'s links`}>
      {(member.public_links || []).map((link, index) => <a href={link.url} target="_blank" rel="noopener noreferrer" key={`${link.type}-${index}`}>{link.label || link.type} <span aria-hidden="true">↗</span></a>)}
    </nav>}<div className="team-activity"><PresenceSummary member={member} />{!member.custom_status && !(member.activities || []).length && "No current activity"}</div></section>
  </>}</main><Footer /></>;
}
