...
export function ModerationView({ guildId, config, csrfToken }: { guildId: string; config: GuildConfig | null; csrfToken?: string }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    saveConfig(guildId, "automod", values, csrfToken).then(() => setState({ saving: false, message: "Moderation settings saved.", error: "" })).catch((error) => setState({ saving: false, message: "", error: error instanceof Error ? error.message : "Could not save settings." }));
  };
...
export function AiView({ guildId, config, csrfToken }: { guildId: string; config: GuildConfig | null; csrfToken?: string }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); setState({ saving: true, message: "", error: "" }); saveConfig(guildId, "ai", values, csrfToken).then(() => setState({ saving: false, message: "AI settings saved.", error: "" })).catch((error) => setState({ saving: false, message: "", error: error instanceof Error ? error.message : "Could not save settings." })); };
...
export function LevelingSettings({ guildId, config, resources, csrfToken }: { guildId: string; config: GuildConfig | null; resources: GuildResources | null; csrfToken?: string }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); setState({ saving: true, message: "", error: "" }); saveConfig(guildId, "leveling", values, csrfToken).then(() => setState({ saving: false, message: "Leveling settings saved.", error: "" })).catch((error) => setState({ saving: false, message: "", error: error instanceof Error ? error.message : "Could not save settings." })); };
