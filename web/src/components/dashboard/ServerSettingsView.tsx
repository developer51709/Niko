...
export function ServerSettingsView({ guildId, config, resources, csrfToken }: { guildId: string; config: GuildConfig | null; resources: GuildResources | null; csrfToken?: string }) {
  ...
  useEffect(() => setValues(initialValues(config?.server)), [config]);
  ...
  const submit = (event: FormEvent) => {
    ...
    saveConfig(guildId, "server", {
      ...
      setValues(initialValues({ ...config?.server, ...(result.config as Partial<ServerConfig>) }));
  ...
  return <>
    ...
    <form onSubmit={submit} className="settings-stack server-settings-stack">
      ...
      <section className="dash-panel settings-panel"><SectionTitle label="Welcome flow" title="Welcome and rules" detail="Choose where new members see your welcome message and rules." icon="users" /><div className="form-grid">
        <Field label="Welcome channel"><select value={welcomeChannel} onChange={(event) => setValue("welcome_channel", event.target.value)}><option value="">Disabled</option>{resources?.channels.map((channel) => <option value={channel.id} key={channel.id}>#{channel.name}</option>)}</select></Field>
        ...
        <Field label="Rules channel"><select value={values.rules_channel} onChange={(event) => setValue("rules_channel", event.target.value)}><option value="">Not configured</option>{resources?.channels.map((channel) => <option value={channel.id} key={channel.id}>#{channel.name}</option>)}</select></Field>
        ...
        <Field label="Role after rules acknowledgment"><select value={values.rules_role_id} onChange={(event) => setValue("rules_role_id", event.target.value)}><option value="">No role</option>{resources?.roles.map((role) => <option value={role.id} key={role.id}>@{role.name}</option>)}</select></Field>
      </div></section>
      ...
      <section className="dash-panel settings-panel"><SectionTitle label="Audit trail" title="Logging destinations" detail="Pick a channel for each event type and disable categories you do not need." icon="book" /><div className="server-logging-list">
        {loggingCategories.map(([key, label]) => <div className="server-logging-row" key={key}><label className="form-field"><span className="form-label">{label} logs</span><select value={String(values.logging[key] || "")} onChange={(event) => setLoggingChannel(key, event.target.value)}><option value="">Not set</option>{resources?.channels.map((channel) => <option value={channel.id} key={channel.id}>#{channel.name}</option>)}</select></label><label className="setting-row compact-setting-row"><span><strong>Enabled</strong><small>{channelLabel(resources, values.logging[key])}</small></span><input type="checkbox" checked={!values.disabled_logging.includes(key)} onChange={() => toggleLogging(key)} /><i aria-hidden="true" /></label></div>)}
      </div></section>
      ...
      <section className="dash-panel settings-panel"><SectionTitle label="Support desk" title="Ticket panel" detail="Configure the public panel and decide who can handle tickets." icon="users" /><div className="form-grid">
        ...
        <Field label="Panel channel"><select value={ticketChannel} onChange={(event) => setValue("panel_channel_id", event.target.value)}><option value="">Keep current panel channel</option>{resources?.channels.map((channel) => <option value={channel.id} key={channel.id}>#{channel.name}</option>)}</select></Field>
        ...
        <Field label="Support roles" hint="Hold Ctrl/Cmd to select more than one"><select multiple value={values.support_roles} onChange={(event) => setValue("support_roles", Array.from(event.target.selectedOptions, (option) => option.value))}>{resources?.roles.map((role) => <option value={role.id} key={role.id}>@{role.name}</option>)}</select></Field>
